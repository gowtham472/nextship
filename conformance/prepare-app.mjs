 ...
 *
 * Linux only: the test's variables are found by comparing two processes'
...
const HARNESS_DIR = '.nextship-e2e'
...
const flags = new Map()
if (process.env.IS_TURBOPACK_TEST) flags.set('IS_TURBOPACK_TEST', '1')
if (process.env.IS_WEBPACK_TEST) flags.set('IS_WEBPACK_TEST', '1')
if (process.env.__NEXT_CACHE_COMPONENTS) {
  flags.set('NEXT_PRIVATE_EXPERIMENTAL_CACHE_COMPONENTS', process.env.__NEXT_CACHE_COMPONENTS)
}
if (process.env.__NEXT_EXPERIMENTAL_CACHED_NAVIGATIONS) {
  flags.set('NEXT_PRIVATE_EXPERIMENTAL_CACHED_NAVIGATIONS', process.env.__NEXT_EXPERIMENTAL_CACHED_NAVIGATIONS)
}

// For the build only, as on Vercel. It compiles in the hydration marker the harness
// waits for; without it every browser page load waited out a 10 second fallback,
// and tests that load several pages ran out of time.
const buildEnv = new Map([...testEnv, ...flags, ['NEXT_PRIVATE_TEST_MODE', 'e2e']])
...
if (/\/test\/e2e\/app-dir\/next-config-ts-native-m?ts\//.test(process.env.TEST_FILE_PATH ?? '')) {
  buildEnv.set('__NEXT_NODE_NATIVE_TS_LOADER_ENABLED', 'true')
}
...
if (existsSync(VENDORED)) lines.push(`tar -xf ${VENDORED} -C node_modules`)
writeFileSync(`${HARNESS_DIR}/build-env.sh`, `${lines.join('\n')}\n`)

const manifest = JSON.parse(readFileSync('package.json', 'utf8'))
const build = manifest.scripts?.build
if (typeof build !== 'string') fail('the harness wrote no build script into package.json')
...
// npm is in every builder image and runs the same script.
const command = build.endsWith(POST_BUILD) ? `${build.slice(0, -POST_BUILD.length)} && npm run post-build` : build
// The same prefix for every app, so apps with the same dependencies still produce
// the same package.json for the image's install layer.
manifest.scripts.build = `. ./${HARNESS_DIR}/build-env.sh && ${command}`
...
const lockfile = existsSync('package-lock.json') ? JSON.parse(readFileSync('package-lock.json', 'utf8')) : null
if (lockfile && manifest.name === undefined && lockfile.name !== 'app') {
  lockfile.name = 'app'
...
/**
 * Variables this deploy script started with that the harness process did not.
 *
 ...
 */
function variablesTheTestSet() {
  const harness = environmentOf(harnessPid)
  const found = new Map()
  for (const [name, value] of environmentOf(scriptPid)) {
