...
**New in 1.1.0.** Verified on a local test server, a Proxmox VM and DigitalOcean Droplets.
Not yet run on Hetzner, on an arm64 machine, on Debian 12, with a certificate for a real
domain, or from the GitHub Actions workflow below: those are the live checklist in the
...
![A recording of nextship server add, which plans and then prepares a fresh Ubuntu server over SSH, then nextship deploy, which builds the app on the server, starts it behind Caddy and prints its URL, then curl loading the live page.](/media/server-deploy.svg)

One uncut run on a fresh DigitalOcean Droplet, at four times speed. The commands are typed
by a script so they can be read; every line of output is what nextship printed.
...
over SSH, which suits a server with little memory. If the build's connection to the
server's Docker drops mid-build, which has happened right after `server add` on a fresh
server, `deploy` says so and builds once more; any other build failure is reported at once.
...
previous one serving; one that succeeds drops no request. Measured on a DigitalOcean
Droplet: 579 of 579 requests and 51 of 51 streamed responses succeeded across a deployment.
