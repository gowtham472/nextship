...
Verified on Apple Silicon in our own end-to-end test: nextship 0.4.4, installed from npm
...
(`storage`, `ReclaimOutcome` detail) are the driver's words too, and so are how `env`,
`logs`, `domain` and `destroy` describe what the target does. DigitalOcean output is
unchanged by this: against the same stubbed API responses, `main` and the VM branch printed
identical output, byte for byte, for 20 read-only and plan-only commands covering deploy,
rollback, logs, env, domains, images, destroy and doctor. The execute paths were not re-run
against a live App Platform app.
...
`env push` and `env rm` rewrite the env file from SSH stdin and start the live image again
through the release sequence, so a variable change drops no request. `domain add` and
`domain rm` change the domains in `app.json` only after the regenerated site validated and
reloaded, so a domain Caddy refused is never recorded as attached; a domain's state is
`live` when a TLS connection to the server with that name presents a certificate the
system trusts. `images prune` removes an image with its stopped containers and its build
volume. `logs --deployment` reads the journal by container name, so a replaced deployment's
output is still there. `destroy` removes the app's containers, volumes, site and state
directory under its lock, and never the server, Caddy, other apps or DNS. `server status`
reads the server and every app on it in two SSH round trips and warns on under 3 GB free,
published ports other than Caddy's, an outdated setup, a pending reboot and an unhealthy
live app. `deploy` refuses under 3 GB free before anything is built.

Verified on the local stand-in: `env push` made a request-time variable visible with 47 of
47 requests answering 200 during the switch; `logs --deployment` returned a replaced
deployment's output; `images prune --keep 1` removed four images and kept the live one;
`domain add` produced a site that redirected the domain to HTTPS while the default address
kept serving; a second project deployed with its own domain beside the first, and
`destroy --images` removed its containers, volumes, image, site and directory while the
