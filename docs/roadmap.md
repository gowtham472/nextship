...
| An install path that needs no prior knowledge | **Done.** `npm install -g nextship-cli`, published with provenance, each release staged and approved with 2FA. Installed that way on a MacBook Pro with an M3 Max by the first outside tester, 0.4.4 deployed a Next.js app to App Platform on the second attempt, after its first hit the health check path bug fixed for the next release |
